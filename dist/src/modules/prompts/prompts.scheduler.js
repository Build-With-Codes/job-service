...
let PromptsScheduler = PromptsScheduler_1 = class PromptsScheduler {
    ...
    resolveIntervalMs() {
        ...
        if (!Number.isFinite(parsed)) {
            this.logger.warn(`Invalid PROMPT_SYNC_INTERVAL_MS="${raw}". Expected milliseconds, for example 300000 for 5 minutes. Falling back to ${fallback}ms.`);
            return fallback;
        }
        if (parsed < minimum) {
            this.logger.warn(`PROMPT_SYNC_INTERVAL_MS=${parsed} is too low for production. Using minimum ${minimum}ms.`);
            return minimum;
        }
        return parsed;
    }
